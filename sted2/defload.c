...
#if 0  /*  Jun.18.2003 Toshi Nagata: delete the following block of code */
...
int	defload(char *fi,int md)
{
  ...
  top:
	...
	#if defined(USE_AQUA)
	{
      ...
      /* Jun.18.2003 Toshi Nagata: delete the following block of code */
...
/*      if ( str_search(li2, "#KEY_INS=")>0 )
  ...
  def_bsave(fi,last_no,mutef,initf);
	...
	/*  2015.3.21. Toshi Nagata  */
	if (def_path[0] == 0) {
		snprintf(def_path, sizeof def_path, "%s/deffile", s_sted_default_path);
	}
	/*  End Toshi Nagata  */
