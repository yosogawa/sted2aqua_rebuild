...
extern int	tkey,tvel;	/* May.26.2003 Toshi Nagata: add 'extern' */
...
/***************************/
/*
  1997-07-25	Trk.Edit§«[OPT1]+[CLR]§«•»•È•√•Ø•·•‚§Ú…Ωº®§π§Î
  •»•È•√•Ø•ª•Ï•Ø•ø§Ú∏∆§”Ω–§ª§Î§Ë§¶§À§∑§ø
  1997-07-26	¢¨§Œ•»•È•√•Ø•ª•Ï•Ø•øæÂ§«§‚[OPT1]+•∆•Û•≠°º§«∞‹
  ∆∞Ω–ÕË§Î§Ë§¶§À§∑§ø
  1997-07-29	Trk.Edit§«§Œ[CTRL]+[T]§«[¢≠]§«•»•È•√•Ø•ª•Ï•Ø•ø
  §Ú∏∆§”Ω–§ª§Î§Ë§¶§À§∑§ø
  */
/*
...
void	fsel(char *fna,char *pth,int w)
{
  ...
  char	tmp0[PATHLEN],exts[PATHLEN];
  X68_DIR	dir[512];
  int	cdirp[32],cdiry[32],cdirc;
	...
	/*  2015.3.27. Toshi Nagata  */
	{
		...
		w = (w & 0xff);
		if (w >= 64) {
			MacErrorBox("Conversion by external package is not yet implemented.", 2);
			return;
		}
		MacSelectFile(fna, pth, (cc & 1), fsel_title[w], fsel_ext[w]);
		es = 0;
		return;
	}
	/*  End Toshi Nagata  */
  ...
  resel2:
    ...
    if( nm==0 || nm==-2 ){nm=0;/*msg("•’•°•§•Î§¨§¢§Í§ﬁ§ª§Û°£");goto reinp;*/}
...
int	fdir(char *pth,char *exts,X68_DIR *dir,int w)
{
  ...
  char	file[PATHLEN],tmp0[PATHLEN],tmp1[PATHLEN];
