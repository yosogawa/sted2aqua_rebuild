...
int    chd_check(int,int,int);
int    tai_check(int,int);
...
{
  char	path2[PATHLEN],tmp0[PATHLEN];
  int	ex;
  int	e=edit_scr;
...
int	rep_match(int f,int f2,int t,int t2,int a,int m)
{
  if((f2&1)==0){
    ...
    /*	if( f==-7 && ( a>=t	&& a<=t2		)){return 1;}*/	/* »œ∞œ		*/

    if(m==0){
      if( f==-7 && (a>=t && a<=t2	)&& a<128){return 1;}	/* »œ∞œ	*/
    }else{
      if( f==-7 && (a>=t && a<=t2	)){return 1;}
    }

    if( f==-8 && (a>=t && a<=t2)&& a>=0x90 && a<0x98){return 1;}	/* »œ∞œ(usrexc)*/

  }else{
    if( f== 0 && a!=t){
      if(m!=0 ||(m==0 && t<128 && a<128)	){return 1;}	/* !πÁ∆±	*/
...
void	same_shift(int spo,int m,int cu)
{
  ...
  if(ec!=0){
    ...
    /*msg("SAME MEAS§¨");B_PRINT(fstr(ec,4));B_PRINT("≤’ΩÍÃµ∏˙§À§ §Í§ﬁ§∑§ø°£");*/
  }
