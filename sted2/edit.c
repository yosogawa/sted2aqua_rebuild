...
int    replicate2(int,int);
void   key_midi_wait(void);
int    option(int,int);
int    option2(int,int,int,int);
int    option22(int,int,int,int);
int    repl_select(int);
void    load_sub(int);
void    save_sub(int);

static void ed_ch_stgt(int, int, int, int, int);
static int    retkey(int);
static int    retkey2(int);
int    mml_inp(int);
...
int	comment_inp(int ad,int cy,int sh)
{
  ...
  if(trk[track][ad]!=0xf6 ||(sh&1)!=0){
    ...
    line_ins(real_po(ad,cy),cy);
	  tmp0[0]=0;B_LOCATE(15,cy+6); /* sinput(tmp0,20); */ line_input(tmp0, 20);  /* 2015.3.27. Toshi Nagata; sinput->line_input */
    ...
    if(len<=20){
		B_LOCATE(15,cy+6); /* sinput(tmp0,20); */ line_input(tmp0, 20); /* 2015.3.27. Toshi Nagata; sinput->line_input */
    }else{
		B_LOCATE(7,1);/* sinput(tmp0,len);*/ line_input(tmp0, len); msg_clr(); /* 2015.3.27. Toshi Nagata; sinput->line_input */
    }
    if(es!=0){return(0);}
  }
